...
.`breakfast01.bnf` へのリンク
link:{sourcesdir}/breakfast01.bnf[]
...
「Railroad Diagram」と呼ばれる、BNFを線路を模した流れ図のように表現する方法がある。
また、その図を生成するツールやWebサービスはいくつも作られている。例えば、次のようなものがある。

[[rrgen01]]Railroad Diagram Generator(1):: https://bottlecaps.de/rr/ui[^]
[[rrgen02]]Railroad Diagram Generator(2):: https://tabatkins.github.io/railroad-diagrams/[^]

=== 演習：Railroad Diagram Generatorerを使ってみる

. `codes` ディレクトリに、XML（やHTML）のコメントの構文のBNFを作成する。
** 下記「 <<comment_bnf.png>> 」を参照。
...
[[comment_bnf.png]]
.各自が作成するべき `comment.bnf` の内容
--
image::comment_bnf.png[]
--

[[comment.bnf]]
[source,xml]
.各自が `comment.bnf` ファイルを作成したかどうかの確認
----
include::{sourcesdir}/comment.bnf[]
----

link:{sourcesdir}/comment.bnf[上記ファイルへのリンク（作成できていれば参照できる）]
...
. 先述の「 <<rrgen01>> 」に、前の演習で作成した `breakfast01.bnf` をLoadする。
...
** まずは、 status を確認する。
** コミットメッセージは「generate railroad diagram for breakfast01.bnf」でよいでしょう。
** 忘れそうなら、プッシュしておいてもよい。

.生成したページへのリンク（保存したページが下記リンクから参照できるか確認しなさい）
link:breakfast01.xhtml[breakfast01.bnfのRailroad Diagramのxhtmlファイル]
...
[NOTE]
--
作製した `breakfast01.xthml` には、入力を解釈したEBNFも表示されている。 +
これが `codes/breakfast02.bnf` と一致していることを確認しておきなさい。
--
...
.上記ファイルへのリンク（上記ファイルを作成して保存できていればこのリンクから参照できるはず）
link:{sourcesdir}/expression01.bnf[^]
...
. Geminiなどを使って「ランダムに5項の四則演算の計算問題を10問作る かっこあり かけるはアスタリスク 割るはスラッシュで表現」などとして、計算式を生成し、ひとつ選びなさい。
** 選んだ式は、4つの演算子をそれぞれ少なくとも1回は使い、カッコも必ず使た式になっていること。
. `codes` ディレクトリにある既存の `ast.txt` をその式で置き換えなさい。
...
[source,text]
.生成した式を `ast.txt` として保存（既存ファイルと置き換える）
----
include::{sourcesdir}/ast.txt[]
----

.上記ファイルへのリンク
link:{sourcesdir}/ast.txt[^]
